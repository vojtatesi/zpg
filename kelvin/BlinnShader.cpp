...
void BlinnShader::applyUniforms(const std::shared_ptr<Camera>& camera,
...
void BlinnShader::applyLight(const std::shared_ptr<Light>& light)
{
    if (light) light->applyToShader(this);
}

const std::string BlinnShader::BlinnFragmentSource = R"(
#version 400 core
...
void main() {
    ...
    fragColor = vec4(ambient + diffuse + specular, uObjectColor.a);
}
)";

const std::string BlinnShader::BlinnVertexSource = R"(
#version 400 core
